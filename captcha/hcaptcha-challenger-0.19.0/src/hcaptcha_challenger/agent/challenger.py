...
class AgentConfig(BaseSettings):
    ...
    GEMINI_API_KEY: SecretStr = Field(
        default_factory=lambda: SecretStr(os.environ.get("GROQ_API_KEY", os.environ.get("GEMINI_API_KEY", ""))),
        description="Groq API Key",
    )
    GROQ_API_KEY: SecretStr | None = Field(
        default=None,
        description="Groq API Key for image classification tasks (binary/select). "
                    "If set, overrides GEMINI_API_KEY for _image_classifier and _spatial_point_reasoner.",
    )
    ...
    def validate_api_key(cls, v: Any) -> str:
        if not v or not isinstance(v, str):
            raise ValueError(
                "API key is required. Set GROQ_API_KEY environment variable "
                "or pass it directly via GEMINI_API_KEY parameter."
            )
        return v
...
class RoboticArm:
    ...
    def __init__(self, page: Page, config: AgentConfig):
        ...
        # If GROQ_API_KEY is set: use it for image classification (binary/select grids),
        # use GEMINI_API_KEY for spatial path reasoning (drag challenges).
        # If not set: use GEMINI_API_KEY for everything (original behavior).
        _gemini_key = self.config.GEMINI_API_KEY.get_secret_value()
        _groq_key = (
            self.config.GROQ_API_KEY.get_secret_value()
    ...
    async def challenge_image_label_binary(self):
        ...
        for cid in range(crumb_count):
            ...
            # Extract challenge text from payload (more reliable than OCR)
            challenge_prompt = None
            if self.captcha_payload:
                try:
                    challenge_prompt = self.captcha_payload.get_requester_question("en")
                    if not challenge_prompt or challenge_prompt == "unknown":
                        challenge_prompt = self.captcha_payload.get_requester_question()
                except Exception:
                    pass

            # Image classification - pass challenge_prompt so classifier splits into 9 cells
...
class AgentV:
    ...
    def __init__(self, page: Page, agent_config: AgentConfig):
        ...
        self._ignored_refresh_count: int = 0  # tracks refreshes for ignored challenge types
    ...
    async def _solve_captcha(self):
        ...
        try:
            ...
            # Challenge type is ignored — exit cleanly, no refresh
            logger.warning(f"Challenge type {challenge_type} is ignored, skipping")
            return
        except Exception as err:
