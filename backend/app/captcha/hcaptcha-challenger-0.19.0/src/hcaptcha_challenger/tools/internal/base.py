...
class Reasoner(ABC, Generic[ModelT, ResponseT]):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            gemini_api_key: API key (Anthropic Claude key).
    ...
    def _create_default_provider(self):
        """Create provider based on API key: AIza* → Gemini, otherwise → Groq."""
        if self._api_key and self._api_key.startswith("AIza"):
            from .providers.gemini import GeminiProvider
            return GeminiProvider(api_key=self._api_key, model=self._model or "gemini-1.5-flash")
        return GroqProvider(api_key=self._api_key, model=self._model or "meta-llama/llama-4-maverick-17b-128e-instruct")
